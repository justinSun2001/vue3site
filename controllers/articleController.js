...
exports.article_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        // Successful, so render.
        res.render('article_detail', { title: results.article.title, article: results.article,  } );
    });
...
exports.article_create_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err); }
        res.render('article_create_form', { title: 'Create Article',authors:results.authors, genres:results.genres,});
    });
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            async.parallel({
                ...
                res.render('article_create_form', { title: 'Create Article',authors:results.authors, genres:results.genres, errors: errors.array() });
            });
            return;
        }
        else {
